...
namespace WTG.System.Windows.Forms.Tests
{
    public class DataGridColumnStyleTests
    {
        ...
        public void GetColumnValueAtRow_PropertyDescriptor_ReturnsExpected(int rowNum, int expected)
        {
            ...
            var style = new SubDataGridColumnStyle(descriptor);
            var context = new BindingContext();
            var dataSource = new List<DataClass> { new() { Value1 = 1 }, new() { Value1 = 2 }, new() { Value1 = 3 } };
        ...
        public void SetColumnValueAtRow_IEditablePropertyDescriptor_ReturnsExpected(int rowNum, int value)
        {
            ...
            var style = new SubDataGridColumnStyle(descriptor);
            var context = new BindingContext();
            var dataSource = new List<EditableDataClass> { new() { Value = 1 }, new() { Value = 2 }, new() { Value = 3 } };
            CurrencyManager source = Assert.IsType<CurrencyManager>(context[dataSource]);
            source.Position = rowNum;
        ...
        public static IEnumerable<object[]> SetDataGridInColumn_TestData()
        {
            ...
            var dataGrid = new DataGrid
            {
                ...
                DataSource = new List<DataClass> { new() { Value1 = 1 } }
            };
        ...
        {
            protected CustomDataGridColumnStyle()
            {
            }

            protected CustomDataGridColumnStyle(PropertyDescriptor prop) : base(prop)
            {
            }

            protected CustomDataGridColumnStyle(PropertyDescriptor prop, bool isDefault) : base(prop, isDefault)
            {
            }
