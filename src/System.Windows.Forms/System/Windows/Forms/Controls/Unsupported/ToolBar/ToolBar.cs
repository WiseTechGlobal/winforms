...
#pragma warning disable CS8632, CSIsNull001, CSIsNull002, IDE1006, IDE0002, CA1510, WFDEV001, CA1805, CA2020, RS0016, SA1005, SA1129, SA1131, SA1206, SA1505, SA1508, SA1518, IDE0031, IDE0036, IDE0059, IDE0073, IDE0078, IDE0270
...
using ToolBarNativeMethods = System.Windows.Forms.LegacyToolBarNativeMethods;
    ...
    public class ToolBar : Control
    {
        ...
        public Size ButtonSize
        {
            get
            {
                if (buttonSize.IsEmpty)
                {
                    ...
                    {
                            int result = (int)PInvokeCore.SendMessage(this, PInvoke.TB_GETBUTTONSIZE);
                        if (result > 0)
                        {
                            return new Size(ToolBarNativeMethods.Util.LOWORD(result), ToolBarNativeMethods.Util.HIWORD(result));
                        }
                    }
        ...
        protected override CreateParams CreateParams
        {
            get
            {
                ...
                cp.ClassName = ToolBarNativeMethods.WC_TOOLBAR;

                // windows forms has it's own docking code.
                //
                    cp.Style |= ToolBarNativeMethods.CCS_NOPARENTALIGN
                                | ToolBarNativeMethods.CCS_NORESIZE;
                // | ToolBarNativeMethods.WS_CHILD was commented out since setTopLevel should be able to work.

                if (!Divider)
                {
                        cp.Style |= ToolBarNativeMethods.CCS_NODIVIDER;
                }

                if (Wrappable)
                {
                        cp.Style |= ToolBarNativeMethods.TBSTYLE_WRAPABLE;
                }

                if (ShowToolTips && !DesignMode)
                {
                        cp.Style |= ToolBarNativeMethods.TBSTYLE_TOOLTIPS;
                }
                ...
                switch (appearance)
                {
                    ...
                    case ToolBarAppearance.Flat:
                        cp.Style |= ToolBarNativeMethods.TBSTYLE_FLAT;
                        break;
                }
                ...
                switch (textAlign)
                {
                    ...
                    case ToolBarTextAlign.Right:
                        cp.Style |= ToolBarNativeMethods.TBSTYLE_LIST;
                        break;
                }
        ...
        internal int PreferredHeight
        {
            get
            {
                ...
                else
                {
                    ...
                    PInvokeCore.SendMessage(this, PInvoke.TB_GETRECT, (WPARAM)firstVisible, ref rect);
                ...
                {
                    height *= unchecked((int)PInvokeCore.SendMessage(this, PInvoke.TB_GETROWS));
                }
            ...
            get
            {
                ...
                if (maxWidth == -1)
                {
                    ...
                    else
                    {
                        ...
                        {
                            PInvokeCore.SendMessage(this, PInvoke.TB_GETRECT, 0, ref rect);
            ...
            if (disposing)
            {
                ...
                lock (this)
                {
                    ...
                    try
                    {
                        ...
                        if (buttonsCollection is not null)
                        {
        ...
        private void ForceButtonWidths()
        {
            if (buttons is not null && buttonSize.IsEmpty && IsHandleCreated)
            {
                ...
                for (int x = 0; x < buttonCount; x++)
                {
                    ...
                    ToolBarNativeMethods.TBBUTTONINFO tbbi = new()
                    {
                        cbSize = Marshal.SizeOf<ToolBarNativeMethods.TBBUTTONINFO>(),
                        cx = buttons[x].Width
                    };
                    ...
                    tbbi.dwMask = (int)ToolBarNativeMethods.TBIF_SIZE;
                    PInvokeCore.SendMessage(this, PInvoke.TB_SETBUTTONINFO, (WPARAM)x, ref tbbi);
                }
            }
        }
        ...
        private void InsertButton(int index, ToolBarButton value)
        {
            ...
            if (IsHandleCreated)
            {
                ToolBarNativeMethods.TBBUTTON tbbutton = value.GetTBBUTTON(index);
                PInvokeCore.SendMessage(this, PInvoke.TB_INSERTBUTTON, (WPARAM)index, ref tbbutton);
            }
        ...
        internal void InternalSetButton(int index, ToolBarButton value, bool recreate, bool updateText)
        {
            ...
            if (IsHandleCreated)
            {
                ToolBarNativeMethods.TBBUTTONINFO tbbi = value.GetTBBUTTONINFO(updateText, index);
                PInvokeCore.SendMessage(this, PInvoke.TB_SETBUTTONINFO, (WPARAM)index, ref tbbi);
                ...
                else
                {
                    ...
                    //
                    PInvokeCore.SendMessage(this, PInvoke.TB_AUTOSIZE);
        ...
        protected override void OnHandleCreated(EventArgs e)
        {
            ...
            //
            PInvokeCore.SendMessage(this, PInvoke.TB_BUTTONSTRUCTSIZE, (WPARAM)Marshal.SizeOf<ToolBarNativeMethods.TBBUTTON>());

            // set up some extra goo
            //
            if (DropDownArrows)
            {
                PInvokeCore.SendMessage(this, PInvoke.TB_SETEXTENDEDSTYLE, 0, (LPARAM)ToolBarNativeMethods.TBSTYLE_EX_DRAWDDARROWS);
            }
            ...
            if (imageList is not null)
            {
                PInvokeCore.SendMessage(this, PInvoke.TB_SETIMAGELIST, 0, imageList.Handle);
            }
                ...
                try
                {
                    ...
                    for (int x = 0; x < buttonCount; x++)
                    {
                        if (buttons[x].Text.Length > 0)
                        {
                            ...
                            buttons[x].stringIndex = PInvokeCore.SendMessage(this, PInvoke.TB_ADDSTRING, 0, addString);
                        }
                        else
                        {
                    ...
                    //
                    int cb = Marshal.SizeOf<ToolBarNativeMethods.TBBUTTON>();
                    ...
                    for (int x = 0; x < count; x++)
                    {
                        ...
                        ToolBarNativeMethods.TBBUTTON tbbutton = buttons[x].GetTBBUTTON(x);
                        Marshal.StructureToPtr(tbbutton, (IntPtr)(checked((long)ptbbuttons + (cb * x))), true);
                        buttons[x].parent = this;
                    }

                    PInvokeCore.SendMessage(this, PInvoke.TB_ADDBUTTONS, (WPARAM)count, ptbbuttons);
        ...
        {
            PInvokeCore.SendMessage(this, PInvoke.TB_SETBUTTONSIZE, 0, ToolBarNativeMethods.Util.MAKELPARAM((int)(buttonSize.Width * currentScaleDX), (int)(buttonSize.Height * currentScaleDY)));
        }
        ...
        internal override void SetToolTip(ToolTip toolTip)
        {
                ...
                PInvokeCore.SendMessage(this, PInvoke.TB_SETTOOLTIPS, (WPARAM)toolTip.Handle);
        ...
        {
            var lParam = m.GetLParam(typeof(ToolBarNativeMethods.NMTOOLBAR)) ?? throw new InvalidOperationException("lparam of a received message should not be null");

            ToolBarNativeMethods.NMTOOLBAR nmTB = (ToolBarNativeMethods.NMTOOLBAR)lParam;

            ToolBarButton tbb = buttons[nmTB.iItem];
            if (tbb is null)
            {
                throw new InvalidOperationException($"ToolBar button at index {nmTB.iItem} was not found.");
            }
            ...
            {
                RECT rc = new RECT();
                LegacyMenuNativeMethods.TPMPARAMS tpm = new();

                PInvokeCore.SendMessage(this, PInvoke.TB_GETRECT, (WPARAM)nmTB.iItem, ref rc);
                ...
                {
                    MainMenu main = menu.GetMainMenu();
                    if (main is not null)
                    {
                        main.ProcessInitMenuPopup(menu.Handle);
                    }

                    PInvokeCore.MapWindowPoints((HWND)nmTB.hdr.hwndFrom, HWND.Null, ref rc);
                    ...
                    LegacyMenuSafeNativeMethods.TrackPopupMenuEx(
                                                  new HandleRef(menu, menu.Handle),
                                                  ToolBarNativeMethods.TPM_LEFTALIGN |
                                                  ToolBarNativeMethods.TPM_LEFTBUTTON |
                                                  ToolBarNativeMethods.TPM_VERTICAL,
                                                  rc.left, rc.bottom,
        ...
        {
            ToolBarNativeMethods.TOOLTIPTEXT ttt = (ToolBarNativeMethods.TOOLTIPTEXT)m.GetLParam(typeof(ToolBarNativeMethods.TOOLTIPTEXT));
            int commandID = (int)ttt.hdr.idFrom;
            ToolBarButton tbb = buttons[commandID];

            if (tbb is not null && tbb.ToolTipText is not null)
            {
            ...
            {
                ttt.uFlags |= (int)TOOLTIP_FLAGS.TTF_RTLREADING;
            }
        ...
        {
            var lParam = m.GetLParam(typeof(ToolBarNativeMethods.NMTBHOTITEM)) ?? throw new InvalidOperationException("lparam of a received message should not be null");

            // Should we set the hot item?
            ToolBarNativeMethods.NMTBHOTITEM nmTbHotItem = (ToolBarNativeMethods.NMTBHOTITEM)lParam;
            if (ToolBarNativeMethods.HICF.HICF_ENTERING == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_ENTERING))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_LEAVING == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_LEAVING))
            {
                hotItem = -1;
            }
            else if (ToolBarNativeMethods.HICF.HICF_MOUSE == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_MOUSE))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_ARROWKEYS == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_ARROWKEYS))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_ACCELERATOR == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_ACCELERATOR))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_DUPACCEL == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_DUPACCEL))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_RESELECT == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_RESELECT))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_LMOUSE == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_LMOUSE))
            {
                hotItem = nmTbHotItem.idNew;
            }
            else if (ToolBarNativeMethods.HICF.HICF_TOGGLEDROPDOWN == (nmTbHotItem.dwFlags & ToolBarNativeMethods.HICF.HICF_TOGGLEDROPDOWN))
            {
                hotItem = nmTbHotItem.idNew;
            }
        }
        ...
        {
            int id = ToolBarNativeMethods.Util.LOWORD(m.WParam);
        ...
        protected override void WndProc(ref Message m)
        {
            switch (m.Msg)
            {
                case (int)MessageId.WM_REFLECT_COMMAND:
                ...
                case (int)MessageId.WM_REFLECT_NOTIFY:
                    ...
                    var lParam = m.GetLParam(typeof(NMHDR)) ?? throw new InvalidOperationException("lparam of a received message should not be null");
                    ...
                    switch (note.code)
                    {
                        ...
                        case PInvoke.TTN_SHOW:
                                ...
                                hotItem != -1)
                            {
                                ...
                                PInvoke.ClientToScreen(HWND, ref leftTop);
                        ...
                        case ToolBarNativeMethods.TBN_HOTITEMCHANGE:
                            WmNotifyHotItemChange(ref m);
                            break;

                        case ToolBarNativeMethods.TBN_QUERYINSERT:
                            m.Result = (IntPtr)1;
                            break;

                        case ToolBarNativeMethods.TBN_DROPDOWN:
        ...
        public class ToolBarButtonCollection : IList
        {
            ...
            public void Clear()
            {
                ...
                for (int x = owner.buttonCount; x > 0; x--)
                {
                    ...
                    {
                        PInvokeCore.SendMessage(owner, PInvoke.TB_DELETEBUTTON, (WPARAM)(x - 1));
                    }
            ...
            public void RemoveAt(int index)
            {
                ...
                if (owner.IsHandleCreated)
                {
                    PInvokeCore.SendMessage(owner, PInvoke.TB_DELETEBUTTON, (WPARAM)index);
                }
