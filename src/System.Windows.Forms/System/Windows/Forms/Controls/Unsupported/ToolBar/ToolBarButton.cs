...
namespace System.Windows.Forms
{
    ...
    public class ToolBarButton : Component
    {
        ...
        internal ToolBarButtonImageIndexer ImageIndexer
        {
            get
            {
                return imageIndexer ??= new ToolBarButtonImageIndexer(this);
            }
        }
        ...
        public bool Enabled
        {
            ...
            set
            {
                if (enabled != value)
                {
                    ...
                    {
                            PInvokeCore.SendMessage(parent, PInvoke.TB_ENABLEBUTTON, (WPARAM)FindButtonIndex(),
                            enabled ? 1 : 0);
                    }
                }
            }
        ...
        public int ImageIndex
        {
            ...
            set
            {
                if (ImageIndexer.Index != value)
                {
                    if (value < -1)
                    {
                        throw new ArgumentOutOfRangeException(nameof(ImageIndex), value, string.Format(SR.InvalidArgument, nameof(ImageIndex), value));
                    }
        ...
        public bool PartialPush
        {
            get
            {
                ...
                {
                    if ((int)PInvokeCore.SendMessage(parent, PInvoke.TB_ISBUTTONINDETERMINATE, (WPARAM)FindButtonIndex()) != 0)
                    {
                        partialPush = true;
                    }
                    else
        ...
        public Rectangle Rectangle
        {
            get
            {
                ...
                {
                    RECT rc = new RECT();
                    PInvokeCore.SendMessage(parent, PInvoke.TB_GETRECT, (WPARAM)FindButtonIndex(), ref rc);
        ...
        internal int GetButtonWidth()
        {
            ...
            ToolBarNativeMethods.TBBUTTONINFO button = new()
            {
                cbSize = Marshal.SizeOf<ToolBarNativeMethods.TBBUTTONINFO>(),
                dwMask = (int)ToolBarNativeMethods.TBIF_SIZE
            };

            int buttonID = (int)PInvokeCore.SendMessage(Parent, PInvoke.TB_GETBUTTONINFO, (WPARAM)commandId, ref button);
            if (buttonID != -1)
            {
        ...
        {
            if ((int)PInvokeCore.SendMessage(parent, PInvoke.TB_ISBUTTONCHECKED, (WPARAM)FindButtonIndex()) != 0)
            {
                pushed = true;
            }
            else
        ...
        /// </summary>
        internal ToolBarNativeMethods.TBBUTTON GetTBBUTTON(int commandId)
        {
            ToolBarNativeMethods.TBBUTTON button = new()
            {
            ...
            if (enabled)
            {
                button.fsState |= (byte)ToolBarNativeMethods.TBSTATE_ENABLED;
            }

            if (partialPush && style == ToolBarButtonStyle.ToggleButton)
            {
                button.fsState |= (byte)ToolBarNativeMethods.TBSTATE_INDETERMINATE;
            }

            if (pushed)
            {
                button.fsState |= (byte)ToolBarNativeMethods.TBSTATE_CHECKED;
            }

            if (!visible)
            {
                button.fsState |= (byte)ToolBarNativeMethods.TBSTATE_HIDDEN;
            }
            ...
            switch (style)
            {
                case ToolBarButtonStyle.PushButton:
                    button.fsStyle = (byte)ToolBarNativeMethods.TBSTYLE_BUTTON;
                    break;
                case ToolBarButtonStyle.ToggleButton:
                    button.fsStyle = (byte)ToolBarNativeMethods.TBSTYLE_CHECK;
                    break;
                case ToolBarButtonStyle.Separator:
                    button.fsStyle = (byte)ToolBarNativeMethods.TBSTYLE_SEP;
                    break;
                case ToolBarButtonStyle.DropDownButton:
                    button.fsStyle = (byte)ToolBarNativeMethods.TBSTYLE_DROPDOWN;
                    break;
        ...
        /// </summary>
        internal ToolBarNativeMethods.TBBUTTONINFO GetTBBUTTONINFO(bool updateText, int newCommandId)
        {
            ToolBarNativeMethods.TBBUTTONINFO button = new()
            {
                cbSize = Marshal.SizeOf<ToolBarNativeMethods.TBBUTTONINFO>(),
                dwMask = (int)(ToolBarNativeMethods.TBIF_IMAGE
                            | ToolBarNativeMethods.TBIF_STATE | ToolBarNativeMethods.TBIF_STYLE)
            };
            ...
            if (updateText)
            {
                button.dwMask |= (int)ToolBarNativeMethods.TBIF_TEXT;
            }
            ...
            if (newCommandId != commandId)
            {
                ...
                button.dwMask |= (int)ToolBarNativeMethods.TBIF_COMMAND;
            }
