...
public class BindingContextTests
{
    ...
    private sealed class InterceptingBindingContext : BindingContext
    {
        ...
        protected override EventHandler? CreateInterceptedParentHandler(CurrencyManager relatedManager)
        {
            return (sender, e) =>
            {
                HandlerInvokeCount++;

                if (sender is CurrencyManager { Count: > 0 })
                {
                    relatedManager.GetType().GetMethod("ParentManager_CurrentItemChanged", BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(relatedManager, [sender, e]);
                }
                else if (_placeholder is not null)
                {
                    relatedManager.GetType().GetMethod("SetDataSource", BindingFlags.Instance | BindingFlags.NonPublic)!.Invoke(relatedManager, [_placeholder]);
                }
            };
        }
    }
