...
#pragma warning restore WFDEV004
    ...
    private static void DisposeOwnedToolStripDropDowns(Control control)
    {
        if (control is ToolStrip toolStrip)
        {
            foreach (ToolStripItem item in toolStrip.Items)
            {
                if (item is ToolStripDropDownItem { HasDropDown: true } dropDownItem
                    && dropDownItem.DropDown.IsAutoGenerated
                    && !dropDownItem.DropDown.IsDisposed)
                {
        ...
        {
            DisposeOwnedToolStripDropDowns(child);
        }
    }
    ...
    protected override void Dispose(bool disposing)
    {
        if (disposing)
        {
            DisposeOwnedToolStripDropDowns(this);
